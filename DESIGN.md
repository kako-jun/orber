...
The same tokens drive the **transparent-DL checkbox** introduced for #56 ("透過版を DL に含める / Include transparent versions"). It sits directly under the aspect toggles (centred, `col-span-2`) so all download-affecting settings cluster together at the top of the grid. The checkbox remains clickable even when the browser cannot encode VP9 alpha (`VideoEncoder.isConfigSupported({codec:'vp09.00.10.08', alpha:'keep'})` is rejected — observed on Safari, Edge / Chrome on Windows, Android Chrome, and many other combinations; see `docs/overview.md` for details), because transparent PNG / WebP for still tiles is independent of VP9 and is still produced. To make the silent skip of animated-tile WebM visible, a small `text-xs text-fgMuted` notice ("⚠ この環境は透過動画に対応していません…" / `alphaVideoUnsupportedNotice`) is rendered directly beneath the checkbox whenever `vp9AlphaSupported()` is false, so the user can see before clicking download that animated transparent clips will not appear in the ZIP. The OFF state is byte-exact with the pre-#56 download path (the alpha worker calls are simply never made).
