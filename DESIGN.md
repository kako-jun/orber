...
The same tokens drive the **transparent-DL checkbox** introduced for #56 ("透過版を DL に含める / Include transparent versions"). It sits directly under the aspect toggles (centred, `col-span-2`) so all download-affecting settings cluster together at the top of the grid. The encoder pipeline went through WebCodecs VP9-alpha (#56, rejected by most environments) → ffmpeg.wasm libvpx-vp9 (#184) → and finally a **JS-only PNG-in-MOV muxer** (`web/src/lib/movMuxer.ts`, since #192): the worker streams per-frame straight-alpha RGBA PNGs and the main thread packs them into a QuickTime MOV (rgba lossless, no encoder to load). There is therefore no capability probe and no encoder-load fallback row anymore — the checkbox is always available and the `alphaVideoUnsupportedNotice` / `alphaEncoderLoadFailed` strings are gone (see `docs/overview.md` "Transparent download bundle" for details). The OFF state is byte-exact with the pre-#56 download path (the alpha render-and-muxer calls are simply never made). Since #245 the transparent frames come from the WebGPU read-back (`gpu_render_rgba`) rather than a WebGL canvas.

**Error surfacing (hard-fail inventory).** Studio routes all batch failures through `setErrorMsg(formatRunBatchError(e, t))` + `setPhase('error')`, mapping worker sentinels to localized strings: `image-shape-no-contrast` → `imageShapeNoContrast`, and (since #245) **`webgpu-unsupported` → `webgpuUnsupported`** ("お使いのブラウザは WebGPU 非対応です / Your browser does not support WebGPU"). The WebGPU case is a hard fail with no fallback — WebGPU is required (#207) — so the message is terminal, not a degraded mode.
