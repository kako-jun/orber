...
- **C. QR コード** — `/orber-qr.svg` (build 前に `qrcode` パッケージで pre-generate して `public/` 配置、bg `#040404` / fg `#FFFFFF`)。`https://orber.llll-ll.com/` を指す
- **D. Copyright** — `© 2026 kako-jun` (`font-display font-light text-xs text-fgSubtle`)
- **E. Nostalgic Counter** — `<nostalgic-counter id="orber-PLACEHOLDER" type="total" format="text" />`。embed は Footer の onMount で `https://nostalgic.llll-ll.com/components/visit.js` を `data-orber-nostalgic` フラグ付きで idempotent に注入。実 ID は kako-jun がダッシュボードで取得後に置換 (TODO コメント明記)
- **Privacy note (#86 統合)** — 「画像はブラウザ内で処理されます。サーバーへの送信はありません。」を `text-fgMuted` で 1 行
...
すべての可視文字列は `web/src/lib/strings.ts` 経由 (`sponsorLabel` / `sponsorTitle` / `affiliateHeading` / `affiliateDisclosure` / `qrLabel` / `qrAlt` / `privacyNote` / `viewsLabel`)。 `© 2026 kako-jun` は固有名詞扱いで素のテキスト。
