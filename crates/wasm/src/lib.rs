...
/// GUI の count 上限（high=25, #265）を十分上回るため当面同一バリデーションで揃えておく。
...
pub struct WasmParams {
    ...
    /// 10/20/25 を spec.count に上書きしてからレンダリングする（#265: high 30→25）。
    #[serde(default)]
...
/// low=10 / mid=20 / high=25 で固定（#265: 「多め」=30 がモバイルで少し重かった
/// ため 30→25 に下げた。標準 20 より多いまま、48→5 タップ削減と併せて軽量化）。
#[cfg(any(target_arch = "wasm32", test))]
...
fn resolve_frame(mut p: WasmParams, n: u32, spec_idx: u32) -> Result<ResolvedFrame, String> {
    ...
    // count_preset (high=25, #265) は GL_RENDERER_MAX_ORBS=64 未満。将来 high を 64 超に
    // 上げるならここを更新する。WGSL canvas-present 経路はデータテクスチャ経路で
    // この上限を必要としないが、GUI の count 上限 25 (#265) を十分上回るため当面同一
    // バリデーションで揃えておく。
...
mod tests {
    ...
    fn parse_count_preset_table() {
        ...
        assert_eq!(parse_count_preset("high").unwrap(), Some(25));
