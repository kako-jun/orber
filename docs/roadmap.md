...
- **Phase B (Web GUI + WebGL Glyph 描画) 完了 ✅** — `55-phase-b-web-gui-glyph` ブランチで実装
  - wasm: `WasmParams` に `glyph_char` / `count_preset` / `speed_preset` / `contrast_preset` を追加。`MotionSpeed::Mid` / `Fast` を wasm 入口に露出（panic を撤去）。新 API `get_glyph_alpha_mask(ch, size)` / `glyph_supported(ch)` を公開
  - core: `glyph::render_glyph_alpha_mask(font, ch, size) -> Vec<u8>` を追加（tiny-skia path fill、`(font, ch, size)` で wasm 側キャッシュ）
  - WebGL fragment shader: `u_glyph_mask: sampler2D` (R8) / `u_shape_id` / `u_alpha_mul` を追加。`shape_id == 1` で texture sampling、Circle 経路は既存のまま
  - Studio.tsx: aspect クリック即生成を廃止、ガチャボタンを唯一の生成トリガーに昇格、折りたたみアドバンスト 4 軸 (shape / glyph 文字 / count / speed / contrast) を追加。Mid + Standard 揃いで Phase A 以前と完全同値
  - DESIGN.md §13 AdvancedSection を追加
