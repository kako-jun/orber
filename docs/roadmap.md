...
- [x] **#239 (PoC 段階)**: aquarelle「水彩（にじみ＋character）」を再設計 — **にじみ(bleed) を本物の
  ...
  `--bloom`/`--halo`/`--offset <weak|mid|strong>`（=0.3/0.6/0.9、`requires=bleed`）、内部の
  `--aquarelle-bleed-mode`/`--aquarelle-bleed <float>` は上級者向けに残存（`--bleed` と排他）。
  Studio Web は「にじみ：なし/弱/中/強」＋「芯の光/縁の彩度/かたより：なし/弱/中/強」のセグメントボタン
  （wasm GPU 経路に配線、にじみ「なし」で 3 軸 disabled）。**残（未完）**: 旧 `--shape aquarelle`
  （別シェーダ `orb_aquarelle.wgsl` の radial 4 層）はまだ共存・本番 Studio 未公開。次フェーズ #233 統合で
  旧 aquarelle shape を削除し、本番公開して close する
- [ ] **#233**: Aquarelle Web 初公開 → #239 に統合 close（aquarelle は A/B 対象外）。
  #239 で水彩機構は orb.wgsl の blur に再設計済み。残りは旧 radial shader（`orb_aquarelle.wgsl`）の削除と本番公開
