...
- [x] **#239 (PoC 段階)**: aquarelle「水彩（にじみ＋character）」を再設計 — **にじみ(bleed) を本物の
  ...
  （wasm GPU 経路に配線、にじみ「なし」で 3 軸 disabled）。
- [x] **#239 Phase 1（モデル統一）**: 旧 `--shape aquarelle`（`orb_aquarelle.wgsl` の radial 4 層）を撤去し、
  ...
  動画の色/keyframe track アニメ(#7/#33)は旧 aquarelle 経路だけが担っていたため dead 化、CLI を正直に（警告＋
  help 注記）し再配線は #251 に退避。#233（Aquarelle Web 初公開）は #239 に統合済みで既に close。
