...
- [ ] **#241**: 薄い影（再スコープ済み・**実装済み、s の最終値は kako-jun 選定待ち**）—
  ...
  `SHADOW_STRENGTH_DEFAULT = 0.3`（仮）+ gpu-lab shadow スライダー
  （`WasmParams.shadow_strength`）で実装済み。残タスク: kako-jun が gpu-lab 実機で
  s を選定 → 定数 1 箇所を差し替え → 実機サインオフで close
