...
| 7 | 動画入力対応（ffmpeg でフレーム抽出） | ❌ 撤去（session611）— 入力は静止画のみに統一。揺らぎ・色変化は入力非依存の procedural 効果（位置サイン揺らぎ / 色相脈動）へ置換し、写真・Web 全経路で効くようにした |
