...
- [ ] **#245 PR-B**: 削除一式 — `web/src/lib/orberGl.ts` + orberGl.test.ts /
  `AbPanel.tsx` / `lib/abLogic.ts` / `lib/webgpu.ts` / `strings.ts` の ab* キー /
  Studio の `?ab=1` 組み込み / `crates/wasm/src/ab_harness.rs` + native dev-dependencies
  （serde_json / orber-core gpu）/ gpu-lab の WebGL 残骸（あれば）/ docs 同期。
  受け入れ: `rg -i webgl` で本文ヒットなし（歴史的記述を除く）
