...
- Web GUI `generate_batch` now pins the **last 5 tiles** to `VariationKind::Mp4` regardless of total count (`still_count = total - 5`). Both the portrait (10 tiles) and landscape (9 tiles) layouts therefore produce 5 video-track tiles in the second half — the still/video ratio no longer drifts when the layout changes. (Animated playback for those tiles is tracked in #50.)
