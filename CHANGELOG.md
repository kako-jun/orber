...
- Per-orb integer speed multipliers (`1x` / `2x` / `3x`) assigned deterministically from the seed. Combined with the `MotionSpeed` cycle count (`VerySlow` / `Slow` / `Medium` = 1 / 2 / 3), effective traversal counts spread over `{1, 2, 3, 4, 6, 9}` per clip — orbs visibly move at varied paces while pixel-exact loop closure at `t=0 ≡ t=1` is preserved (integer × integer cycles). (#41)
