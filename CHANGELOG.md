...
### Added
- Web GUI advanced controls (#55 Phase B). The Studio surface gains a collapsible "Advanced / アドバンスト" panel between the aspect toggles and the gacha button. When opened it exposes four monochrome segmented controls + an inline 1-character input:
  - **Shape**: Circle (existing radial gradient) / Glyph (single character outline fill, drawn via the same Noto Sans Symbols 2 subset that the CLI already ships). Selecting Glyph reveals a 1-char input that defaults to `☆`. Characters outside the bundled font set are flagged with a fg-muted "not in bundled font" notice but the gacha button stays enabled (the orb is silently skipped, matching the CLI's tofu-suppression behaviour).
  - **Count**: Few (10) / Standard (20) / Many (35).
  - **Speed**: Slow / Standard / Fast — the CLI's full four-step ladder (very-slow / slow / mid / fast) collapsed to three for the GUI; very-slow is intentionally not surfaced.
  - **Contrast**: Soft / Standard / Strong — Soft is tuned for sitting under text overlays, Strong for standalone wallpaper use, Standard is **byte-exact identity** with Phase A's previous output.
  Aspect (Portrait / Landscape) toggles no longer auto-rerun the batch — they only flip the state. The gacha button below the advanced section is now the **single source of generation**, with the previous reload-arrow icon promoted to a labelled "Roll / ガチャを引く" chip with a slightly larger touch target. New SolidJS signals: `shape`, `glyphChar`, `glyphCharSupported`, `countPreset`, `speedPreset`, `contrastPreset`, `advancedOpen`. The three preset signals default to `''` (empty string) rather than `'mid'` so the wasm boundary's identity path keeps Phase A's `random_batch_specs` count distribution (10..=50) and the four-tile `GUI_VIDEO_SPEEDS` split intact; the "Standard" segmented button shows as pressed for both `''` and `'mid'` so the UI looks unchanged but never accidentally pins every tile to a single value. Documented in DESIGN.md §13. (#55 Phase B)
- `orber-wasm` exposes `get_glyph_alpha_mask(ch, size) -> Uint8Array` and `glyph_supported(ch) -> bool`. The mask is a `size × size` row-major alpha buffer baked from the bundled font's outline path via the new `orber_core::glyph::render_glyph_alpha_mask` (tiny-skia path fill, anti-aliased). Both the wasm wrapper and the worker cache the mask per `(ch, size)` so repeated calls in a batch never re-traverse the font or re-upload the texture. (#55 Phase B)
- `WasmParams` gains four optional string fields used by the Web GUI: `glyph_char` (1 char, required when `shape == "glyph"`), `count_preset` (`""` / `"low"` / `"mid"` / `"high"` → respect / 10 / 20 / 35; `""` and `"mid"` both map to "use spec.count" via the GUI's identity convention), `speed_preset` (`""` / `"slow"` / `"mid"` / `"fast"` — `""` and `"mid"` both mean "use spec.speed and the `GUI_VIDEO_SPEEDS` override"; `very-slow` is **CLI-only** and rejected on the wasm boundary so the GUI cannot accidentally smuggle it in past the 3-value UI), and `contrast_preset` (`""` / `"low"` / `"mid"` / `"high"` — empty / mid is `ContrastPreset::Mid` identity). All four default to `""` via `#[serde(default)]` so existing wasm callers stay byte-compatible. The previous `shape: "circle"` only restriction is lifted — `"glyph"` is now accepted alongside it. (#55 Phase B)
...
- `--count-preset low|mid|high` — shorthand alternative to `--count <N>`, mapped to `10 / 20 / 35`. Mutually exclusive with `--count`. (#55)
...
- `--contrast low|mid|high` — single-axis preset that bundles alpha, blur, and edge sharpness. `low` (weak alpha + strong blur + soft edges) is tuned for sitting beneath text overlays; `high` (reduced blur + sharper rim, alpha held at `mid` to keep the `mid = identity` invariant) is tuned for standalone wallpaper-style use; `mid` is the **identity preset** — its values are exactly the previous defaults, so existing renders are bit-for-bit unchanged. A core regression test pins the `mid = identity` invariant. New `ContrastPreset { Low, Mid, High }` enum lives in `crates/core/src/style.rs`. (#55)
