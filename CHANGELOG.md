...
### Added
- `--shape glyph` — render each orb as a single text character instead of a round blob. Pick the character with `--glyph-char <CHAR>` (default `☆`, exactly one character). Glyphs are drawn via `ttf-parser` outline extraction + `tiny-skia` path fill against a bundled **Noto Sans Symbols 2 subset** (~177 KB, `include_bytes!` from `crates/core/assets/fonts/NotoSansSymbols2-Regular.ttf`) covering ASCII, digits, punctuation, arrows, and geometric shapes. Glyphs outside the subset (emoji etc.) are silently skipped instead of rendering tofu. `--blur` is ignored in this mode (outline fill, no gaussian post-pass). New `OrbShape::Glyph { ch, font: GlyphFontId }` + `GlyphFontId` enum keep `OrbShape: Copy` by routing the `ttf_parser::Face<'static>` through a process-global `OnceLock` cache keyed by enum variant (rather than per-orb `Arc<Face>`). New dep `ttf-parser = "0.25"`. (#55)
...
- `--contrast low|mid|high` — single-axis preset that bundles alpha, blur, and edge sharpness. `low` (weak alpha + strong blur + soft edges) is tuned for sitting beneath text overlays; `high` (strong alpha + reduced blur + sharper rim) is tuned for standalone wallpaper-style use; `mid` is the **identity preset** — its values are exactly the previous defaults, so existing renders are bit-for-bit unchanged. A core regression test pins the `mid = identity` invariant. New `ContrastPreset { Low, Mid, High }` enum lives in `crates/core/src/style.rs`. (#55)
